            ...
            if (request()->ajax()) {
                return datatables()->of(User::with('profile')->whereNull('closed_on')->where('role_id', 2)->get())
                    ...
                    ->addColumn('action', function ($data) {
                        return '<button title="Delete" type="button" name="delete" id="' . $data->id . '" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></button>&nbsp<a href="'.route('admin.user.userPosts', $data->id).'" title="User Post" type="button" id="' . $data->id . '" class="btn btn-primary btn-sm">User Posts</a>';
